...
export interface IRepository<T extends Aggregate> {
    new (id: string, version: number, state: null): T;
  }
...
class Repository<T extends Aggregate> {
    ...
    async get(id: string, ctor: IRepository<T>): Promise<T> {
        const client = await this.pool.connect();
        ...
        const aggregate: T = new ctor('id123', 3, null);
    ...
    async save(aggregate: Aggregate): Promise<void> {
        ...
        for (var i = 0; i < aggregate.changes.length; i++) {
            ...
            const query = `INSERT INTO event_stream VALUES ('${event.aggregateId}', ${event.aggregateVersion}, 'TodoListCreated', '${JSON.stringify(event.payload)}')`;
